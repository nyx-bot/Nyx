module.exports = {
    "name": "bruh",
    "desc": "play a bruh sound effect in your voice channel.",
    "args": [],
    "aliases": [
        "bruhsound"
    ],
    ...
    func: async function(ctx, msg, args) {
        ...
        let file = `./res/sound-effects/bruh.mp3`
        let chances = [true, false, false, true, false, false, false, false, false];
        if(chances[Math.floor((Math.random() * chances.length))] === true) {
            let rareSounds = []
            ctx.libs.fs.readdir('./res/sound-effects/bruh/', (err, files) => {
                file = `./res/sound-effects/bruh/${files[Math.floor((Math.random() * files.length))]}`
            });
        }
