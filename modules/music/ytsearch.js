module.exports = {
    "name": "ytsearch",
    "desc": "Search on YouTube for a song!",
    "args": [
        {
            "opt": false,
            "arg": "song to look up"
    ...
    func: async function ytsearch(ctx, msg, args) {
        ...
        ctx.libs.superagent.get(`${ctx.musicApi.location}search/youtube/${encodeURI(args.join(' '))}`).set('auth', `${ctx.musicApi.key}`).then(r => r.body.results).then(async r => {
            ...
            r.forEach(async res => {
                ...
                if(r.length === 1) {
                    ...
                    let pos = await ctx.music[msg.channel.guild.id].addTrack([`${artists[0]} - ${title}`, duration, url, msg.author.id, thumbnail, id, `yt`, ctx.utils.randomGen(16)])
                    delete ctx.music[`search${msg.author.id}-${msg.channel.guild.id}`] 
            ...
            if(r.length !== 1) {
                ctx.music[`search${msg.author.id}-${msg.channel.guild.id}`].m = await m.edit({
                    content: `\`\`\`css\n${result}\`\`\`\n${ctx.yt} ${ctx.utils.music.playText(msg.prefix, count)}`,
                    components: [
                        {
                            type: 1,
                            components: [
