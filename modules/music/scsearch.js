module.exports = {
    "name": "scsearch",
    "desc": "Search on SoundCloud for a song!",
    "args": [
        {
            "opt": false,
            "arg": "song to look up"
    ...
    func: async function scsearch(ctx, msg, args) {
        try {
        ...
        if(ctx.utils.music.regex.soundcloud.test(args[0]) || ctx.utils.music.regex.scplaylist.test(args[0])) return ctx.cmds.get('play').func(ctx, msg, args);
        let m = await msg.reply(`${ctx.processing} Searching for **${ctx.utils.escapeDiscordsFuckingEditing(args.join(' '))}**...`)
        ctx.libs.superagent.get(`${ctx.musicApi.location}search/soundcloud/${encodeURI(args.join(' '))}`).set('auth', `${ctx.musicApi.key}`).then(r => r.body.results).then(async r => {
            ...
            ctx.music[`search${msg.author.id}-${msg.channel.guild.id}`] = {source: `sc`, results: [], author: msg.author.id, timeout: 30000};
            ...
            r.forEach(async res => {
                ...
                if(r.length === 1) {
                    if(Number(duration[0]) > msg.author.maxtime) return m.edit(`${ctx.fail} ${ctx.sc} I was unable to add ${ctx.utils.escapeDiscordsFuckingEditing(artist + ` - ` + title)}! (You can't play songs longer than ${(await ctx.utils.timeConvert(msg.author.maxtime))})`)
                    try {
                        m = await ctx.utils.music.joinChannel(ctx, msg, m);
                    } catch(e) {return}
                    let pos = await ctx.music[msg.channel.guild.id].addTrack([`${artist} - ${title}`, duration, url, msg.author.id, thumbnail, id, `sc`, ctx.utils.randomGen(16)])
                    delete ctx.music[`search${msg.author.id}-${msg.channel.guild.id}`] 
                    return await m.edit(await ctx.utils.music.getMusicCard(ctx, `${m.content} added ${ctx.sc} **"${ctx.music[msg.channel.guild.id].queue[pos][0]}"** \`[${ctx.utils.timestampConvert(ctx.music[msg.channel.guild.id].queue[pos][1][0] || ctx.music[msg.channel.guild.id].queue[pos][1])}]\` to the queue, starting ${pos === 0 ? `**now**` : pos === 1 ? `**after this song**` : `at position **${pos}**`}!`, {
            ...
            if(r.length !== 1) {
                ctx.music[`search${msg.author.id}-${msg.channel.guild.id}`].m = await m.edit({
                    content: `\`\`\`css\n${result}\`\`\`\n${ctx.sc} ${ctx.utils.music.playText(msg.prefix, count)}`,
                    components: [
                        {
                            type: 1,
                            components: [
