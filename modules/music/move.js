module.exports = {
    "name": "move",
    "desc": "Move a track's position!",
    "args": [
        {
            "opt": false,
            "arg": "original track position, or name of first track"
        },
        {
            "opt": false,
            "arg": "new position to place the track."
        }
    ],
    "permission": 3,
    "aliases": [
        ...
        "options": [
            {
                "type": 3,
                "name": "track",
                "description": "The current position or name of the song.",
                "required": true
            },
            {
                "type": 4,
                "name": "position",
                "description": "The new position for the song",
                "required": true
            }
        ]
    },
    func: async function move (ctx, msg, args) {
        ...
        if(isNaN(args[0]) || isNaN(args[1])) {return msg.reply(`${ctx.fail} Usage: \`${msg.prefix}${this.usage.replace(';', '')}\``)} else {
            let orig = Math.round(args[0]);
            let newpos = Math.round(args[1]);
            if(orig > ctx.music[msg.channel.guild.id].queue.length || orig < 1) {
                return msg.reply(`${ctx.fail} You can only move tracks between **1 - ${ctx.music[msg.channel.guild.id].queue.length}**!`)
            } else if(newpos < 1) {
                return msg.reply(`${ctx.fail} You can only move tracks between **1 - ${ctx.music[msg.channel.guild.id].queue.length}**!`)
            } else {
                if(orig === newpos) return msg.reply(`${ctx.fail} You can only move tracks when the designated position is not the same as the original!`)
                if(newpos > ctx.music[msg.channel.guild.id].queue.length) newpos = ctx.music[msg.channel.guild.id].queue.length;
                let song = ctx.music[msg.channel.guild.id].queue.splice(orig, 1)[0];
                ctx.music[msg.channel.guild.id].queue.splice(newpos, 0, song)
                msg.reply(`${ctx.pass} Successfully moved **${ctx.utils.escapeDiscordsFuckingEditing(song[0])}** to position ${newpos} in the queue.`);
